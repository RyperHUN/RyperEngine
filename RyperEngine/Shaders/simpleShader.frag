...
vec3 calcSpotLight (SpotLight light, vec3 wFragPos, vec2 texCoord, Material mat)
{
	...
	vec3 texturedColor = light.color * mat.kd * texture(texture_diffuse, texCoord).xyz ;
...
vec3 calcDirLight(DirLight light, vec3 normal, vec3 viewDir, vec2 texCoord, Material mat)
{
	...
	vec3 diffuse  = diff * light.color * mat.kd *texture(texture_diffuse, texCoord).xyz;
	vec3 specular = spec * light.color * mat.ks *texture(texture_specular, texCoord).xyz;
...
vec3 calcPointLight (PointLight light, vec3 normal, vec3 viewDir, vec3 wFragPos, vec2 texCoord, Material mat)
{
	...
	vec3 diffuse  = light.color * diff * mat.kd * texture(texture_diffuse, texCoord).xyz;
	vec3 specular = light.color * spec * mat.ks * texture(texture_specular, texCoord).xyz;
	diffuse  *= attenuation; 
